...
| mac Apple Silicon 原生 arm64 | 曾 0/155 FAIL, 两层根因均已自建补丁修复: ①**apelink 在 shell ELF 头硬编码 e_flags=0** → mac m1 loader TryElf 视作非 APE-modern 改写 argv[0] (applet 名丢失): `patches/cosmo/cosmo-apelink-apeflags-extra.patch`。②修复①后剩 loader 深度嵌套 STANDALONE exec 走 PATH 原生工具 (argv0=applet 名使 busybox 找不到自身): **GetProgramExecutableName 在 loader 形态解析 loader argv 载荷路径** `patches/cosmo/cosmo-pen-mac-loader-extra.patch` + busybox `get_busybox_exec_path()` 改调惰性解析 (messages.c)。loader 形态代理实测 deep-test 31/31、smoke-full 173/0 | ✅ 自建补丁 (2026-09-04); macos-15 arm64 CI 真机复验 |
