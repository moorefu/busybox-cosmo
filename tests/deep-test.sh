#!/bin/sh
# deep-test.sh — busybox APE 深度测试(补充 smoke)
# 主战场: Linux 容器(podman)与 Windows; 建议: busybox.com sh deep-test.sh
# 覆盖: awk/sed 深度、长管道、大文件、并发 fork 压力
PASS=0
FAIL=0
SKIP=0

. "$(dirname "$0")/testlib.sh"
bbtest_init deep

t() {
	desc="$1"
	shift
	if bbtest_run "$@"; then
		echo "PASS: $desc"
		PASS=$((PASS + 1))
	else
		echo "FAIL: $desc"
		FAIL=$((FAIL + 1))
	fi
}

echo "===== busybox APE deep test ====="

# --- awk 深度 ---
t "awk 字段+NF" sh -c 'echo "a b c d e" | awk "{print NF}" | grep -q 5'
t "awk 数组聚合" sh -c 'printf "x 1\nx 2\ny 3\n" | awk "{s[\$1]+=\$2} END{print s[\"x\"]}" | grep -q 3'
t "awk 正则+sub" sh -c 'echo "abc123def" | awk "{gsub(/[0-9]+/, \"N\"); print}" | grep -q abcNdef'
t "awk BEGIN/END" sh -c 'echo "l1" | awk "BEGIN{n=0} {n++} END{print n}" | grep -q 1'
t "awk printf 格式" sh -c 'echo "3.14159" | awk "{printf \"%.2f\\n\", \$1}" | grep -q 3.14'
t "awk 多条件分支" sh -c 'seq 1 20 2>/dev/null | awk "\$1%2==0{print \"e\"} \$1%2==1{print \"o\"}" | sort | uniq -c | wc -l | grep -q 2'
t "awk 字符串函数" sh -c 'echo "Hello World" | awk "{print tolower(\$0)}" | grep -q "hello world"'
t "awk getline" sh -c 'printf "1\n2\n3\n" | awk "{getline x; print x}" | grep -q 2'

# --- sed 深度 ---
t "sed 多命令" sh -c 'echo "abc" | sed "s/a/X/; s/c/Z/" | grep -q XbZ'
t "sed 地址范围" sh -c 'printf "1\n2\n3\n4\n" | sed "2,3d" | wc -l | grep -q 2'
t "sed 行追加" sh -c 'echo "a" | sed "a\\b" | wc -l | grep -q 2'
t "sed 保持空间" sh -c 'printf "1\n2\n" | sed -n "1h;2{g;p}" | grep -q 1'
t "sed -i 原地" sh -c 'echo "old" > sed-i.txt && sed -i "s/old/new/" sed-i.txt && grep -q new sed-i.txt && rm -f sed-i.txt'

# --- 长管道/数据流 ---
t "8 级管道链" sh -c 'echo "a b c d e f" | tr " " "\\n" | sort -r | head -3 | tail -1 | tr a-z A-Z | sed "s/^/X/" | grep -q XD'
t "5MiB 数据流 SHA256 已知答案" sh -c 'set -o pipefail; awk "BEGIN { for(i=0;i<5242880;i++) printf \"x\" }" | sha256sum | grep -q "^dba67a476fa78973aabb087f214a1010f3bebca053674e0af50dfe5a582112be "'
t "tee 分流" sh -c 'echo flow | tee tee1.txt | grep -q flow && grep -q flow tee1.txt && rm -f tee1.txt'
# [cosmo 限制] mkfifo/mknod: cosmo 未实现 mknodat wrapper(ENOSYS), 不测

# --- 大文件 ---
# 用确定性普通文件隔离 cp/hash/压缩测试；/dev/zero 在 smoke-full 中单独断言，
# 这样设备读取失败不会伪装成四个无关 applet 同时回归。
t "生成 20MiB 确定性数据" sh -c 'awk '\''BEGIN { s="0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef"; for(i=0;i<327680;i++) printf "%s",s }'\'' > big.bin && test "$(wc -c < big.bin)" -eq 20971520'
t "20MiB cp 校验" sh -c 'cp big.bin big2.bin && cmp big.bin big2.bin && rm -f big2.bin'
t "大文件 sha 一致性" sh -c 's1=$(sha256sum big.bin | cut -d" " -f1) && s2=$(cat big.bin | sha256sum | cut -d" " -f1) && test -n "$s1" && test "$s1" = "$s2"'
t "大文件 gzip 往返" sh -c 'gzip -c big.bin > big.gz && gzip -dc big.gz | cmp - big.bin && rm -f big.gz'
t "大文件 tar 往返" sh -c 'mkdir -p big.d && cp big.bin big.d/f && tar czf big.tgz big.d && tar xzf big.tgz -O big.d/f | cmp - big.bin && rm -rf big.d big.tgz'

# --- 并发 fork 压力 ---
t "xargs -P8 并发" sh -c 'seq 1 16 | xargs -P8 -n1 sh -c "echo \$\$ >> conc.txt; sleep 0.05" && wc -l < conc.txt | grep -qx 16 && rm -f conc.txt'
t "10 个后台进程" sh -c 'i=1; pids=""; while [ $i -le 10 ]; do (sleep 0.1; echo bg-$i >> bg.txt) & pids="$pids $!"; i=$((i+1)); done; for pid in $pids; do wait "$pid" || exit 1; done; test "$(wc -l < bg.txt)" -eq 10 && rm -f bg.txt'
t "50 次 fork 循环" sh -c 'i=1; while [ $i -le 50 ]; do sh -c "exit 0" || exit 1; i=$((i+1)); done; echo fork50-ok'
t "嵌套子 shell 3 层" sh -c 'x=$(echo $(echo $(echo deep))) && test "$x" = deep'
t "并发生产者" sh -c '(yes p1 | head -2000 > cp1.txt) & p1=$!; (yes p2 | head -2000 > cp2.txt) & p2=$!; wait "$p1" && wait "$p2" && test "$(wc -l < cp1.txt)" -eq 2000 && test "$(wc -l < cp2.txt)" -eq 2000 && rm -f cp1.txt cp2.txt'
if sh -c 'cat <(echo procsub) 2>/dev/null | grep -q procsub' >/dev/null 2>&1; then
	t "进程替换式 fd" sh -c 'cat <(echo procsub) 2>/dev/null | grep -q procsub'
else
	echo "SKIP: 进程替换式 fd (当前 shell 不支持 process substitution)"
	SKIP=$((SKIP + 1))
fi

# --- 文本大输入 ---
t "grep 万行" sh -c 'seq 1 10000 | grep -c 999 | grep -q 19'
t "sort 五千行稳定" sh -c 'awk "BEGIN { for(i=5000;i>=1;i--) print i }" | sort -n > sorted && test "$(head -1 sorted)" = 1 && test "$(tail -1 sorted)" = 5000 && test "$(wc -l < sorted)" -eq 5000'
t "wc 大输入" sh -c 'yes word | head -100000 | wc -w | grep -qx 100000'

# --- 交互工具可用性(无 tty 探测) ---
t "ed 脚本化编辑" sh -c 'printf "a\\nhello-ed\\n.\\nw ed.txt\\nq\\n" | ed >/dev/null 2>&1 && grep -q hello-ed ed.txt && rm -f ed.txt'
t "vi 二进制存在" sh -c 'vi --help 2>&1 | grep -qi usage'

echo "===== 结果: $PASS passed, $FAIL failed, $SKIP skipped ====="
[ "$FAIL" -eq 0 ]
